...
\title{\Large 16-833: Robot Localization and Mapping, Spring 2026\\
\textbf{\Large Homework 3 - Linear and Nonlinear\\ SLAM Solvers}}
\maketitle
\begin{flushright}
\textbf{\uline{Due: Friday Mar 27, 11:59pm, 2026}}
\par\end{flushright}
...
The answer is given with the assumption that the input for $h_o$ has shape $\begin{bmatrix} r_x^t & r_y^t & r_x^{t+1} & r_y^{t+1} \end{bmatrix}^T$, and similarly, the input for $h_l$ has shape $\begin{bmatrix} r_x^t & r_y^t & l_x^k & l_y^k \end{bmatrix}^T$.

\[ h_o(\mathbf{r}^t,~ \mathbf{r}^k) = \begin{bmatrix} r_x^{t+1} - r_x^t \\ r_y^{t+1} - r_y^t \end{bmatrix} \] 

\[ H_o(\mathbf{r}^t,~ \mathbf{r}^k) = \begin{bmatrix} -1 & 0 & 1 & 0 \\ 0 & -1 & 0 & 1 \end{bmatrix}\]

\[ h_l(\mathbf{r}^t,~ \mathbf{l}^k) = \begin{bmatrix} l_x^k - r_x^t \\ l_y^k - r_y^t \end{bmatrix} \] 
...
\begin{tabular}{|c|c|c|}
\hline
    Method & Runtime (s) & Visualization \\ \hline
    pinv & 1.4062423706054688 & \\ \hline
    lu & 0.015847206115722656 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{lu_linear.png} \\ \hline
    lu\_colamd & 0.02504420280456543 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{lu_colamd_linear.png} \\ \hline
    qr & 0.2497105598449707 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{qr_linear.png} \\ \hline
    qr\_colamd & 0.17293643951416016 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{qr_colamd_linear.png} \\ \hline
\end{tabular}
\end{center}
\vspace{1em}
pinv is the slowest since it doesn't exploit sparisty and must invert a large matrix (that it thinks is dense). Cholesky is the fastest non-colamd solutions, which is an advantage over QR that we've discussed in class since it works over $A^TA$ and not $A$ and $A^TA$ is smaller because the problem is overdetermined. Regarding colamd, we see a decrease in performance for Cholesky and and increase in performance for QR factorization.
...
\hline
Method & Runtime (s) & Visualization \\ \hline
    pinv & 0.11315441131591797 & \\ \hline
    lu & 0.029034852981567383 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{lu_loop.png} \\ \hline
    lu\_colamd &  0.01010894775390625 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{lu_colamd_loop.png} \\ \hline
    qr & 0.2979743480682373 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{qr_loop.png} \\ \hline
    qr\_colamd & 0.026876449584960938 & \rule{0pt}{5cm} \includegraphics[width=0.4\textwidth]{qr_colamd_loop.png} \\ \hline
\end{tabular}
\end{center}
\vspace{1em}
There are differences in performance, the most prominent one being that colamd has a significant performance improvement for both Cholesky and QR now. This is because the loop closures leads to more non-zero entries during the factorization processes; by reordering the rows, colamd decreases the amount of extra non-zero entries.
...
Let $\Delta_x = l^k_x - r^t_x, \Delta_y = l^k_y - r^t_y, d = \sqrt{\Delta_x^2 + \Delta_y^2}$

\begin{align*}
H_l(\mathbf{r}^t, \mathbf{l}^{k}) &= \begin{bmatrix} 
\frac{1}{1 + (\frac{\Delta_y}{\Delta_x})^2} \cdot \frac{\Delta_y}{\Delta_x^2} & \frac{1}{1 + (\frac{\Delta_y}{\Delta_x})^2} \cdot \frac{-1}{\Delta_x} & \frac{1}{1 + (\frac{\Delta_y}{\Delta_x})^2} \cdot \frac{-\Delta_y}{\Delta_x^2} & \frac{1}{1 + (\frac{\Delta_y}{\Delta_x})^2} \cdot \frac{1}{\Delta_x}\\
\frac{-\Delta_x}{\sqrt{\Delta_x^2 + \Delta_y^2}} & \frac{-\Delta_y}{\sqrt{\Delta_x^2 + \Delta_y^2}} & \frac{\Delta_x}{\sqrt{\Delta_x^2 + \Delta_y^2}} & \frac{\Delta_y}{\sqrt{\Delta_x^2 + \Delta_y^2}}\\
\end{bmatrix} \\
&= \begin{bmatrix}
\frac{\Delta_y}{d^2} & \frac{-\Delta_x}{d^2} & \frac{-\Delta_y}{d^2} & \frac{\Delta_x}{d^2} \\
\frac{-\Delta_x}{d} & \frac{-\Delta_y}{d} & \frac{\Delta_x}{d} & \frac{\Delta_y}{d}
\end{bmatrix}
\end{align*}
...
\subsubsection{Solution}
\begin{tabular}{|c|c|}
\hline Pre-optimized QR & Post-optimized QR \\ \hline
\rule{0pt}{6cm}\includegraphics[width=0.5\textwidth]{nonlinear_qr_preopt.png} & \rule{0pt}{6cm}\includegraphics[width=0.5\textwidth]{nonlinear_qr_postopt.png} \\ \hline
\end{tabular}

\hspace{1em}

Linear SLAM solves in one go: you build a linear system once and solve directly for x. Nonlinear SLAM is solved in a for loop. Each cycle of the for loop uses a different point to do linearization around, computes a correction $dx$ to apply to the state, then updates the state.

\section{Code Submission}
Instructions:
\begin{itemize}
    \item Install \href{https://docs.astral.sh/uv/}{$\mathtt{uv}$} if you haven't already. Then, from the code directory, run $\mathtt{uv\ sync}$ to create a virtual environment and install all dependencies automatically. If you encounter failures with $\mathtt{sparseqr}$, please install SuiteSparse to your system (usually already installed), see \href{https://github.com/yig/PySPQR#dependencies}{dependencies for sparseqr}.
    ...
    {\sffamily uv~run~linear.py~--data~../data/2d\_linear.npz~--method~qr\_colamd}
    \\
